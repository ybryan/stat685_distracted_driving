...
```{r friendly_data, echo=FALSE, message=FALSE, warning=FALSE, fig.width=3, fig.height=4}
...
nd_nest <- nd_df %>% group_by(Subject) %>% tidyr::nest() %>%
    inner_join(subject_bio, by=c("Subject" = "subject")) %>%
...
```{r}
writeLines(readLines(here("src", "stan", "nd_no_lanechange.stan")))
```
