...
- Pooling forces variability of the $\sigma_0$ and $\delta_{j_0}$ to zero as variability is now within all the drivers
...
p1 <- mcmc_intervals(
  ...
  scale_y_discrete(
    ...
    labels=c(
      bquote("Hyperprior for \ndriver variation ("~sigma[i]~")"), 
      bquote("Hyperprior for \ndriver texting effect ("~delta[i]~")")
      )) + 
...
p2 <- mcmc_intervals(
  ...
  scale_y_discrete(
    ...
    labels=c(
      bquote("Lane variation intercept ("~sigma[0]~")"),
      bquote("Texting effect intercept ("~delta[0]~")")
      )) +
  theme(text = element_text(size=16))
bayesplot_grid(p2, p1, xlim=c(0, 0.25))
