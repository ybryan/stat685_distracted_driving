...
- Pooling forces variability of the $\delta_{j_0}$ to zero as variability is now within all the drivers

- Hyperpriors show that the texting effect doubles the lane position variance for the individual drivers
...
posterior_pooled <- as.array(fit_pooled)
p1 <- mcmc_intervals(
  posterior_pooled, pars=c("sigma_base", "delta_base", 
  ...
  scale_y_discrete(
    ...
    labels=c(bquote("Lane variation intercept ("~sigma[0]~")"),
             bquote("Texting effect intercept ("~delta[0]~")"),
             bquote("Hyperprior for driver variation ("~sigma[i]~")"), 
             bquote("Hyperprior for driver texting effect ("~delta[i]~")"))) +
  theme(axis.text.y = element_text(size=12))
p1
rm(p1, fit_pooled, posterior_pooled)
```
:::
::::::::::::::
