...
- Age effect appears to have significant interaction effect with texting 
...
$$\sigma = \sigma_0 * exp \big[log (\sigma_{j}) + log(\sigma_{j, k}) \big]$$
...
$$\sigma_i, \delta_{j, i} \sim \textrm{Half-normal}(0, \sigma_\gamma)$$
$$\sigma_\gamma \sim \textrm{Half-normal}(0, 0.1)$$
...
plot(0, type= "n", 
     ...
     main = "Posterior Predictive Check (y_{t+1})")
t <- md_half$Distance
...
- Clusters of population exist where the effect of texting and the base standard deviation of lane offset seem to match
...
bayesplot_grid(p1, p2,
               titles=c("LaneOffset Std Dev", "Texting Effect"),
...
- Pooling forces variability of the $\delta_{j_0}$ to zero but variability is now within all the drivers

- Hyperpriors show that the texting effect doubles the standard deviation of lane positioning for the individual drivers
...
fit_pooled <- readRDS(
  file=here('etc', 'fits', 'ar1_texting_multi_pooled.RData')) 
posterior_pooled <- as.array(fit_pooled)
mcmc_intervals(posterior_pooled, pars=c("sigma_base", "delta_base", "sigma_driver_sigma", "delta_texting_driver_sigma"))

rm(md_half_all, subject_df, input_data, fit_pooled, posterior_pooled)
```
:::
::::::::::::::
...
* Much longer right tails in the lane position variance
...
dimnames(posterior_age)$parameters[8:9] <- c("Old_Drivers", "Young_Drivers")
p1 <- mcmc_areas(posterior_age,
                 pars=c("Old_Drivers", "Young_Drivers")) +
  xlim(-0.0448, 1.08)

posterior_age <- as.matrix(fit_age)
dimnames(posterior_age)$parameters[10:11] <- c("Old_Drivers", "Young_Drivers")
p2 <- mcmc_areas(posterior_age,
                 pars=c("Old_Drivers", "Young_Drivers")) +
  xlim(-0.0448, 1.08)
bayesplot_grid(p1, p2, titles=c("Lane position variance: Texting", "Lane position variance: Not texting"))
```
:::
::::::::::::::
