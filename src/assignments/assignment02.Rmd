...
tp_subjects <- stringr::str_sub(tp_files, 71, 74)
...
df <- df %>% 
    ...
    dplyr::inner_join(subject_bio, by=c("Subject" = "subject")) %>%
    dplyr::inner_join(psych_df, by=c("Subject"= "subject"))

nd_df <- df %>% dplyr::filter(Drive == "ND") 
md_df <- df %>% dplyr::filter(Drive == "MD") 

feather::write_feather(nd_df, here::here("study_df", "nd.feather"))
feather::write_feather(md_df, here::here("study_df", "md.feather"))
feather::write_feather(df, here::here("study_df", "df.feather"))
feather::write_feather(subject_bio, here::here("study_df", "subject_bio.feather"))
feather::write_feather(psych_df, here::here("study_df", "psych.feather"))
