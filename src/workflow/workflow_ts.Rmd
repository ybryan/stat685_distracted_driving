...
# 1.0 Single driver (T021): Univariate Data
...
### 1.1.2 Define observations

$$K: \textrm{# of AR terms (1)}$$
$$N: \textrm{Number of observations}$$
$$y_n: \textrm{Lane Position (meters)}$$

```{r 1.1.2}
writeLines(readLines(here("src", "stan", "arK.stan"), n=4))
```

### 1.1.3 Identify relevant summary statistics

Correlogram as summary statistic

### 1.1.4 Build a generative model (prior & likelihood)

In order to stay stationary, $\rho$ should be below 1 and should be positive as the driver moves outside the middle of lane to regress back to the mean. Prior for $\rho$ should be below 1. Half-normal prior with standard deviation = `0.324` allows that only 0.1% of the prior probability mass be above $\rho$ = 1

$$\rho \sim \mathcal{H}(0.324^2)$$
$$\alpha \sim \mathcal{N}(1.825, 0.05^2)$$
$$\sigma \sim \mathcal{H}(2^2)$$

```{r 1.1.4a}
rho <- seq(0, 1.5, 0.001)

plot(rho, dnorm(rho, 0, 0.4293), type="l", col=c_dark_highlight, lwd=2,
     xlab=expression(rho), ylab="Prior Density", yaxt='n')

rho99 <- seq(0, 1, 0.001)
dens <- dnorm(rho99, 0, 0.4293)
rho99 <- c(rho99, 1, 0)
dens <- c(dens, 0, 0)
...
### 1.1.5 Analyze the generative ensemble
```{r 1.1.5a, echo=TRUE}
R <- 1000
y <- nd_half$Lane.Position; N <- length(y);
simu_data <- list("N" = N)
fit <- stan(here("src", "stan", "gen_arK.stan"), 
            data=simu_data, iter=R, warmup=0, chains=1, refresh=R, seed=4838282,
...
rm(R, B, counts, cred, idx, x, pad_cred)
```
...
tryCatch({
    ...
    fit_model <- stan_model(here::here("src", "stan", "ar1.stan"))
    ensemble_output <- foreach(simu=simu_list, .combine='cbind') %dopar% {
        simu_rho <- simu[1]
        simu_ys <- simu[2:(N + 1)];
        
        # Fir the simulated observation
        input_data <- list("N" = N, "y" = simu_ys)
        ...
        sbc_rank <- sum(simu_rho <- extract(fit)$rho[seq(1, 4000 - 8, 8)])
        ...
        post_sd_rho <- s[, 3]
        prior_sd_rho <- 0.329
        z_score <- abs((post_mean_rho - simu_rho) / post_sd_rho)
        shrinkage <- 1 - (post_sd_rho / prior_sd_rho)**2
        c(warning_code, sbc_rank, z_score, shrinkage)
    }
}, finally={stopImplicitCluster()})
```

## 1.2 AR(1) of Texting driving

# 2.0 All drivers: Longitudinal [repeated measures] data