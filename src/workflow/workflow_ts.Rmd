...
```{r 1.1.2}
writeLines(readLines(here("src", "stan", "ar1.stan"), n=4))
```
...
```{r 1.1.5a, echo=TRUE, cache=FALSE}
R <- 1000
...
tryCatch({
  ...
  simu_list <- t(data.matrix(data.frame(simu_rhos, simu_ys, simu_alphas)))
...
if (sum(warning_code) != 0) {
  ...
  warnings <- data.frame(warning_code=ensemble_output[1,]) %>%
    group_by(warning_code) %>% summarize(n())
  util$parse_warning_code(warnings$warning_code)
} else {
...
plot(0, type= "n", 
     ...
     xlab = "Distance", ylab = "y", main = "AR(1) Model - Normal Driving")
t <- 1:input_data$N
...
idx <- rep(1:B, each=2)
x <- sapply(1:length(idx), function(b) if(b %% 2 == 0) idx[b] + 0.5 else idx[b] - 0.5)
pad_obs <- do.call(cbind, lapply(idx, function(n) obs_acf[n + 1]))

# TODO
counts <- sapply(1:4000, function(r) acf(simu_ys[r,], plot=FALSE)$acf)
