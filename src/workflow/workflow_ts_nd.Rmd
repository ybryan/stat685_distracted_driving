...
Sample ACF
```{r save_all_data}
md_half_all <- df_1 %>% dplyr::filter(Drive=="MD")
saveRDS(md_half_all, file=here('md_lane_all.RData'))
```
...
non_zero <- simu_rhos != 0
simu_rhos <- simu_rhos[non_zero]
simu_ys <- simu_ys[non_zero, ]
```
...
if (length(bad_r) > 0) {
  if (length(bad_r) > 1) {
    points(apply(simu_ys[bad_r,], 1, mean), simu_rhos[bad_r], col=green, pch=16, cex=0.8)
  } else {
