...
simu_alphas <- extract(fit)$alpha_texting
...
plot(1, type="n", main="Prior Predictive Correlogram of Log variances", 
...
tryCatch({
  ...
  ensemble_output <- foreach(simu=simu_list,
    ...
    s <- summary(fit, probs = c(), pars='alpha_texting[1]')$summary
    post_mean_sigma <- s[,1]
    post_sd_sigma <- s[,3]
    prior_sd_sigma <- 1 
    z_score_sigma <- abs((post_mean_phi - simu_phi) / post_sd_phi)
    ...
    s <- summary(fit, probs = c(), pars='alpha_texting[2]')$summary
    post_mean_sigma <- s[,1]
    post_sd_sigma <- s[,3]
    prior_sd_sigma <- 1 
    z_score_sigma <- abs((post_mean_phi - simu_phi) / post_sd_phi)
    ...
    c(warning_code,
      ...
      sbc_rank_alpha_1, z_score_alpha_1, shrinkage_alpha_1,
      sbc_rank_alpha_2, z_score_alpha_2, shrinkage_alpha_2
      )
  }
