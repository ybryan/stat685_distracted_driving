...
agg_data_stim <- md_half_all %>% group_by(Subject, Stimulus) %>% 
  summarize(sd = sd(LaneOffset)) %>%
  left_join(subject_df, by="Subject")
ggplot(agg_data, aes(type_ab, sd)) + geom_point()
ggplot(agg_data, aes(tai, sd)) + geom_point()
ggplot(agg_data_stim, aes(gender, sd)) + geom_violin() + facet_grid(. ~ Stimulus)
ggplot(agg_data_stim, aes(age_group, sd)) + geom_violin() + facet_grid(. ~ Stimulus)
```
...
fit_multi_age <- stan(here('src', 'stan', 'ar1_texting_multi_age.stan'), data=input_data)
saveRDS(fit_multi, file=here('etc', 'fits', 'ar1_texting_multi.RData'))
saveRDS(fit_multi_age, file=here('etc', 'fits', 'ar1_texting_age.Rdata'))
```
...
mcmc_intervals(posterior_pooled,
