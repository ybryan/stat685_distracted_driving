...
ggplot(md_half, aes(Distance, LaneOffset)) +
    ...
    scale_color_manual(labels=c("No Distraction", "Texting"),
                       values=c(colors$c_mid_highlight,
                                colors$d_mid_highlight))
...
par_select <- dimnames(fit_pooled)$parameters[dimnames(fit_pooled)$parameters %in% par_names]
dimnames(posterior_pooled)$parameters[
...
dimnames(posterior_age)$parameters[8:11] <- c("sigma_texting_old", "sigma_texting_young", "sigma_non_texting_old", "sigma_non_texting_young")
mcmc_areas(posterior_age, regex_pars = c("^sigma_texting", 
                                         "^sigma_non_texting"), prob=0.8)
```