...
# Univariate
```{r graph_md_acf}
df_md_half1 <- readRDS(file=here::here('etc', 'cleaner_data', 'md_half_df.RData'))
...
ciline <- qnorm((1 - conf.level)/2)/sqrt(length(df_md_t021_half1$Lane.Position))
...
ggsave(filename=here::here('etc', 'plots', '02_acf.png'),
       plot = md_acf,
       width=8, height=6)
saveRDS(df_md_t021_half1, file=here::here('etc', 'cleaner_data', 'md_half_t021.RData'))
...
## 3. Summary statistics
```{r prelim_analysis}
md_half_all <- readRDS(here('md_lane_all.RData'))
...
p3 <- ggplot(agg_data_stim, aes(gender, sd)) + geom_violin() + facet_grid(. ~ Stimulus)
p4 <- ggplot(agg_data_stim, aes(age_group, sd)) + geom_violin() + facet_grid(. ~ Stimulus)
...
## 6. Fit observations
```{r multi_fit}
fit_multi <- stan(here('src', 'stan', 'ar1_texting_multi.stan'), data=input_data)
saveRDS(fit_multi, file=here('etc', 'fits', 'ar1_texting_multi.RData'))
```

```{r multi_eval, eval=FALSE}
plot(fit_multi_age, pars=c("sigma_base", "delta_base"))
posterior_multi <- as.matrix(fit_multi_age)
plot_title <- ggtitle("Posterior distributions",
                      "with medians and 80% intervals")
mcmc_areas(posterior_multi, 
           pars = c("sigma_base[1]", "sigma_base[2]",
                    "delta_base[1]", "delta_base[2]"), 
           prob = 0.8) + plot_title
```
...
## 6. Fit observations
```{r, results="hide"}
fit_pooled <- stan(here('src', 'stan', 'ar1_texting_multi_pooled.stan'), data=input_data)
fit_pooled_age <- stan(here('src', 'stan', 'ar1_texting_multi_age_pooled.stan'), data=input_data)
saveRDS(fit_pooled, file=here('etc', 'fits', 'ar1_texting_multi_pooled.RData'))
saveRDS(fit_pooled_age, file=here('etc', 'fits', 'ar1_texting_multi_age_pooled.RData'))
```
...
## Pooled vs regularized
```{r, sigma_driver}
drivers_old <- which(subject_df$age_group == "Old")
drivers_new <- which(subject_df$age_group == "Young")

par_interest <- "sigma_driver"
par_names <- sprintf("%s[%d]", par_interest, drivers_new)

pars_sigma_driver <- dimnames(fit_multi)$parameters[
  str_detect(dimnames(fit_multi)$parameters, pars_select)]
par_names <- dimnames(fit_multi)$parameters[dimnames(fit_multi)$parameters %in% pars_select]

mcmc_intervals(posterior_multi, pars=pars_sigma_driver) + ggtitle("Multi")
mcmc_intervals(posterior_pooled, pars=par_names) + ggtitle("Pooled")
```
...
```{r}
multi_ppcs <- extract(fit_multi, pars = c("sigma_texting", "sigma_non_texting"))
pooled_ppcs <- extract(fit_pooled, pars = c("sigma_texting", "sigma_non_texting"))

md_half_all %>% group_by(Subject, gender, age_group, tai, type_ab) %>% 
  summarize(sd = sd(LaneOffset))
```