...
create_zip <- function(r, hash, simu_list, stan_file) {
  ...
  rds_path <- here("src", "stan",  paste(strsplit(stan_file, "[.]")[[1]][1], ".rds", sep=""))
  stan_path <- here("src", "stan", stan_file)
  r_script_path <- here("src", "docker", 
                        stan_name, paste(strsplit(stan_file, "[.]")[[1]][1], ".R", sep=""))
  ...
  #put_object(zip_file_path, object=s3_key, bucket="stat685-batch") 
}

dir.create(paste("/tmp/zip_files/", hash, "/", sep=""))
1:5 %>% map(function(x) create_zip(x, hash, simu_list, stan_file))
```


**Fit the simulated observations and evaluate
```{r fit_sim_obs, eval=FALSE}
